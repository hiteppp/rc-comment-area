...
export default () => {
  ...
  const loadMoreHandler = () =>
    new Promise((r) => {
      ...
      if (pn.current >= (pageSum.current - 1)) return;
      pn.current++;
  ...
  const PullToLoadMore = () => {
    console.log("pullToPage.current,pn.current", pullToPage.current,pn.current);
    if (canPullToFresh.current) {
      ...
      fetchMoreData(pullToPage.current).then((res) => {
       // pullToPage.current = pullToPage.current > 0 ? pullToPage.current - 1 : 0
        pullToPage.current--
        pn.current--;
  ...
  return (
    <div>
      ...
      <PullToRefresh onRefresh={PullToLoadMore}>
        <div style={{ width: "100%",display:'flex',flexDirection:'row'}} ref={inputRef}>
      ...
      {!isInputVisible && <Space.Compact style={{ width: "100%",position:'sticky',bottom:0 }}>
          <Input  placeholder="请输入评论" />
          <Button type="primary">Submit</Button>
      </Space.Compact>}
      <InfiniteScroll
